import React from 'react'
import { useState } from 'react'

function Signin({setaccess, setuser}) {
    const [email, setemail]=useState()
    const [pass, setpass]= useState()
    const [change,setchange]= useState(false)
    const [fname,setfname] = useState("")
    const [lname,setlname] = useState("")

    const [users,setusers]= useState([{id:1, email:"user@user.com", password:"123", name:"Reporter Reportson",
    status:"...",field:"...", bio:"..." ,
    pic:"data:image/png;base64,"} ])

    const login=(e)=>{
        e.preventDefault()
        var accountexist=false
        var temp={}
      users.forEach(function(user){
        if(user.email== email){
          accountexist=true;
          temp= user
        }
      })
       if(!accountexist){
        alert("account does not exist pleas signup first")
       }else if(pass!= temp.password){
        alert("incorect password")
       }else{
        setaccess(true)
        setuser(temp)
       }
       setemail("")
       setpass("")

      }

      const flip=()=>{
        setchange(!change)
      }

      const signup=(e)=>{
        e.preventDefault()
        const na= fname+" "+lname
        const id= Math.floor(Math.random()*100000)+1;
        const temp={id:id, email:email,password:pass, name:na,
        status:"...",field:"...", bio:"..." ,
        pic:"data:image/png;base64,"}
        setusers({...users,temp})
        setuser(temp)
        setaccess(true)
      }


  return (
    <>
   {!change && <div className="signe-in" >
	<form onSubmit={login}>
  <label htmlFor="E-Mail" className="red">E-Mail</label>  
  <input type="text" id="E-Mail" name="E-Mail" value={email} onChange={(e)=>setemail(e.target.value)} required/>
  <label htmlFor="Password" className="red">Password</label>
  <input type="text" id="" name="Password" value={pass} onChange={(e)=>setpass(e.target.value)} required/>
  <input className="button" type="Submit" value="login" name="in"/>
  <label onClick={flip} style={{marginLeft:"85%",color:'#b30000' }}><b>signup</b></label>
  </form>
  </div>}

  {change &&
  	<form className="signe-in" onSubmit={signup} style={{marginTop:"3.5%"}}>
      <label Htmlfor="firstname" class="red" style={{marginLeft:"40%"}}>First Name</label> 

<input type="text" id="firstname" name="firstname" class="clear"  value={fname} onChange={(e)=>setfname(e.target.value)} required />

<label Htmlfor="lastname" class="red" style={{marginLeft:"40%"}}>Last Name</label>  

<input type="text" id="lastname" name="lastname" class="clear"  value={lname} onChange={(e)=>setlname(e.target.value)} required />

<label Htmlfor="E-Mail" class="red" style={{marginLeft:"40%"}}>E-Mail</label>  

<input type="email" id="email" name="E-Mail" class="clear"  value={email} onChange={(e)=>setemail(e.target.value)} required />

<label Htmlfor="Password" class="red" style={{marginLeft:"40%"}}>Password</label>

<input type="password" id="pass" name="Password" class="clear"  value={pass} onChange={(e)=>setpass(e.target.value)} required />


<input class="button" type="submit" value="Sign up" name="go" />
<label onClick={flip} style={{marginLeft:"85%",color:'#b30000' }}><b>signin</b></label>
</form>}

<div style={{height: "30vh"}}></div>
</>
  )
}

export default Signin